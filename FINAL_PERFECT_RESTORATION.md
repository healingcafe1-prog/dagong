...
2. **여주** - 우아한 백자의 전통을 계승하는 도자기 마을
3. **이천** - 세계적으로 유명한 도자기 예술의 본고장
4. **진천** - 전통 도자기와 공예의 명맥을 이어가는 지역
5. **청주** - 청화백자의 명성을 이어가는 공예의 도시
6. **문경** - 전통 사기와 도자기의 명맥을 잇는 공예의 고장
7. **김해** - 가야 도자기 전통을 잇는 공예의 고장
8. **강진** - 고려청자의 발상지, 천년 도자 예술의 본향
9. **부안** - 변산반도의 전통 옹기와 도자 공예의 고장
...
### 5단계: 공예산지 순서 변경 (완료 ✅)
- regions 테이블에 display_order 컬럼 추가
- 공예산지 순서: 경기 광주 → 여주 → 이천 → 진천 → 청주 → 문경 → 김해 → 강진 → 부안
- API 쿼리 수정: ORDER BY display_order, id
...
- `reorder_craft_regions.sql` - 공예산지 순서 변경
...
npx wrangler d1 execute webapp-production --remote --file=./reorder_craft_regions.sql
...
- ✅ 선물세트: 4개
- ✅ 차산지: 8개
- ✅ 공예산지: 9개 (경기 광주→여주→이천→진천→청주→문경→김해→강진→부안)
- ✅ 총 27개 카테고리
- ✅ 총 17개 지역
- ✅ 모든 데이터 복원
- ✅ API 정상 작동
